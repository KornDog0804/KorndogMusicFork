...
val KORNDOG_THEME_CSS = """ :root { --yt-spec-base-background: #120020 !important; --yt-spec-raised-background: #1a0a2e !important; --yt-spec-menu-background: #2d1450 !important; --yt-spec-brand-background-solid: #2d1450 !important; --yt-spec-general-background-a: #120020 !important; --yt-spec-general-background-b: #1a0a2e !important; --yt-spec-general-background-c: #2d1450 !important; --yt-spec-call-to-action: #39ff14 !important; --yt-spec-static-brand-red: #39ff14 !important; --yt-spec-text-primary: #f0eaf8 !important; --yt-spec-text-secondary: #bba7d9 !important; --yt-spec-icon-active-other: #39ff14 !important; --yt-spec-brand-icon-active: #39ff14 !important; }

ytmusic-nav-bar, #nav-bar-background, ytmusic-player-bar { background: #2d1450 !important; }

ytmusic-player-bar { border-top: 3px solid #39ff14 !important; box-shadow: 0 -2px 18px rgba(57,255,20,.18) !important; }

ytmusic-player-bar .title, .content-info-wrapper .title, ytmusic-player-page .title { color: #39ff14 !important; }

ytmusic-player-bar .subtitle, .content-info-wrapper .subtitle, ytmusic-player-page .subtitle { color: #bba7d9 !important; }

tp-yt-paper-slider #progressContainer #primaryProgress { background: #39ff14 !important; }

ytmusic-chip-cloud-chip-renderer, ytmusic-responsive-list-item-renderer:hover, tp-yt-paper-listbox, ytmusic-menu-popup-renderer { background: #2d1450 !important; }

ytmusic-player-page, ytmusic-browse-response, ytmusic-section-list-renderer { background: #120020 !important; color: #f0eaf8 !important; } """.trimIndent().replace("\n", " ").replace("'", "\'")

val KORNDOG_QUEUE_TRACKER_SCRIPT = """ (function() { if (window._kdQueueTrackerInit) return; window._kdQueueTrackerInit = true;

var QUEUE_STORAGE_KEY = 'korndog_queue'; var MAX_QUEUE_SIZE = 5; var lastSong = ''; var lastProgressSend = 0;

function cleanText(t) { return (t || '') .replace(/\s+/g, ' ') .replace(/Explicit/g, '') .replace(/Album/g, '') .replace(/Song/g, '') .replace(/Video/g, '') .trim(); }

function upgradeThumb(url) { if (!url) return ''; return url .replace(/=w[0-9]+-h[0-9]+.*$/i, '=w800-h800-l90-rj') .replace(//s[0-9]+//i, '/s800/'); }

function getCurrentSongInfo() { var title = ''; var artist = ''; var thumb = ''; var seconds = 0;
...
function getQueue() { try { var stored = localStorage.getItem(QUEUE_STORAGE_KEY); return stored ? JSON.parse(stored) : []; } catch(e) { return []; } }

function saveQueue(queue) { try { localStorage.setItem(QUEUE_STORAGE_KEY, JSON.stringify(queue)); } catch(e) {} }

function sendToAndroid(info) { try { if (!info.title || !info.artist) return;
...
function sendProgress() { try { var media = document.querySelector('video, audio'); if (!media) return;
...
function addToQueue(info) { if (!info.title || !info.artist) return;
...
function checkForNewSong() { try { var info = getCurrentSongInfo(); var songKey = info.title + '|' + info.artist;
...
setTimeout(checkForNewSong, 500); setTimeout(checkForNewSong, 2000); setTimeout(checkForNewSong, 5000); })(); """.trimIndent()

val KORNDOG_CAST_SCRIPT = """ (function() { function applyKornDogThemeAgain() { try { var s = document.getElementById('korndog-theme'); if (s) s.remove();
...
applyKornDogThemeAgain(); setInterval(applyKornDogThemeAgain, 2500);

function initTVButton() { if (!document.body) { setTimeout(initTVButton, 500); return; }
...
function openKornDogGenerator() { try { var queue = []; try { queue = JSON.parse(localStorage.getItem('korndog_queue') || '[]'); } catch(e) {}
...
if (!window._kdSmartKeepAliveInit) { window._kdSmartKeepAliveInit = true;
...
document.addEventListener('waiting', function(e) {
  if (e.target && (e.target.tagName === 'VIDEO' || e.target.tagName === 'AUDIO')) {
    if (!window._kdUserPaused) window._kdShouldBePlaying = true;
  }
}, true);
...
document.addEventListener('canplay', function(e) {
  ...
  if (
    ...
    window._kdShouldBePlaying &&
    !window._kdUserPaused
  ) {
    safePlay(media);
  }
}, true);
...
setInterval(function() {
  try {
    ...
    if (src) window._kdLastKnownGoodSrc = src;
    ...
    var stuck = window._kdShouldBePlaying &&
      !window._kdUserPaused &&
      !ended &&
      !timeMoved &&
      now - window._kdLastMediaCheck > 3500;

    var buffering = window._kdShouldBePlaying &&
      !window._kdUserPaused &&
      !ended &&
      ready < 3;
...
val KORNDOG_SYNCED_LYRICS_SCRIPT = """ (function() { if (window._kdLyricsHighlightInit) return; window._kdLyricsHighlightInit = true;

function injectStyle() { var old = document.getElementById('korndog-lyrics-highlight-style'); if (old) old.remove();
...
injectStyle(); setInterval(injectStyle, 3000);

function visible(el) { if (!el) return false; var r = el.getBoundingClientRect(); return r.width > 0 && r.height > 0; }

function findLyricsContainer() { var selectors = [ 'ytmusic-description-shelf-renderer', 'ytmusic-player-section-list-renderer', 'ytmusic-player-page [role="tabpanel"]', '.lyrics-wrapper', '.lyrics' ];
...
function wrapLines(container) { if (!container || container._kdLyricsWrapped) return; container._kdLyricsWrapped = true;
...
function highlightClosestLine() { var lines = Array.from(document.querySelectorAll('.korndog-lyric-line')); if (!lines.length) return;
...
setInterval(function() { try { var container = findLyricsContainer(); if (container) { wrapLines(container); highlightClosestLine(); } } catch(e) {} }, 500); })(); """.trimIndent()
